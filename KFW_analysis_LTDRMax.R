...
dta_Shp@data["NDVIslopeChange_95_10"] <- dta_Shp$MaxL_2010 - dta_Shp$MaxL_1995
#dta_Shp@data["NDVIslopeChange_01_10"] <- dta_Shp@data["post_trend_NDVI_95_10"] - dta_Shp@data["pre_trend_NDVI_max"]
...
#dta_Shp@data["NDVIslopeChange_01_10"] <- dta_Shp@data["post_trend_NDVI_95_01"] - dta_Shp@data["pre_trend_NDVI_max"]
...
# Make a binary for early vs. late
# dta_Shp@data["TrtBin"] <- 0
...
#Scale all of the data to get standardized coefficients, create psm_PairsB for use in simple OLS models
...
analyticModelEarly3 <- "NDVIslopeChange_95_01 ~ TrtBin+ pre_trend_NDVI_max + MaxL_1995 + terrai_are + Pop_B + MeanT_B  + post_trend_temp +
